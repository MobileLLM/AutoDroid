...
#     def _make_prompt_bk(self, state_prompt, action_history, is_text, state_str, view_text=None, thought_history=None, use_thoughts=False):
#         if self.use_memory:
#             if isinstance(state_str, list):
#                 if len(state_str) == 1:
#                     state_str = state_str[0]
#                 else:
#                     state_str = self.memory.hash_state(state_prompt)
#             new_state_prompt = self._get_item_prediction(action_history, state_prompt, state_str)
#             if new_state_prompt != None and new_state_prompt != 'no_description':
#                 state_prompt = new_state_prompt
#         if use_thoughts:
#             history_with_thought = []
#             for idx in range(len(action_history)):
...
#         introduction = f'You are an agent that can help me complete tasks on a smart phone. Given a task description, \
# the descriptions for all elements on current UI screen, and the action histories you have completed, \
# you predict the id of the UI element on this screen to perform the task, and the thought.'
#         # task_prompt = f"You are using {self.app} on a smartphone to {self.task}."
#         # state_prefix_prompt = 'the current GUI screen has the following elements:'
#         state_prefix_prompt = 'UI elements on the current screen: '
#         # history_prompt = f'You have completed the following steps, which should not be performed again: \n ' + ';\n '.join(action_history)
#         history_prompt = f'Action history:\n' + ';\n '.join(history_with_thought) + ';'
#         # question = 'Which action should you choose next? Just return the element id and nothing else.\nIf no more action is needed, return -1.'
#         task_prompt = f'Task: {self.task}'
#         example = 'For example, you are Given: Task: create a new event type of deadlines.\n App: Calendar\n UI elements on the current screen: "<button id=0 checked=True>Regular event</button>\n <button id=1 checked=False>Add a new type</button>\n<button id=2 checked=False>Manage event types</button>\n<button id=3 class=\'ImageButton\'>go back</button>"\nYou should return: \n\{ID: 1. Thought: In order to add a new event type \'deadlines\', the answer should be to click \'Add a new type\' button to open the \'create new event\' interface and further define the event.\}\nHistory:\n- start the app Calendar;\n- <button class=\'Settings\' checked=False></button>.click();\n- <button checked=False>Manage event types</button>.click()'
#         if not is_text:
#             question = 'If you think the task has not yet be finised, return the button you want to choose or the input box you want to edit in the format of { ID: <element_id>. \
# Thought: <the reason why you choose this element> }. Otherwise, please return in the format of {-1, Thought: <the reason why you think the task has been finished>}\
# Note that you should not choose the ID of elements with <p> </p>.'
#         else:
#             question = f'What text should you enter to the {view_text}? ' + 'Please answer strictly follow the format of: InputText: <text>' #'and thought in the format of: {Text: <input_text> Thought: <the reason why you choose to input this text>}.'
#         prompt = f'{introduction}\n{task_prompt}\n{state_prefix_prompt}\n{state_prompt}\n{history_prompt}\n{question}'
#         return prompt

# class Offline_explore_policy(UtgBasedInputPolicy):

#     def __init__(self, device, app, random_input, task, use_memory=False, debug_mode=False, explore_mode=False):
#         super(Offline_explore_policy, self).__init__(device, app, random_input)
#         self.logger = logging.getLogger(self.__class__.__name__)
#         self.task = task
...
#             action, candidate_actions, target_view = self._get_action_from_views_actions(views=whole_state_views, candidate_actions=whole_state_actions, state_strs=whole_state_strs, action_history=self.__action_history)
...
#             '''
#         else:
#             action, candidate_actions, target_view = self._get_action_with_LLM(current_state, self.__action_history)
...
#         URL = "http://43.153.136.115:8001/beta"
#         data = {
#             "model": "gpt-3.5-turbo",
...
#         answers = requests.post(url=URL, data=data)
...
#     def _make_prompt(self, state_prompt, action_history, is_text, state_str, view_text=None):
#         if self.use_memory:
#             if isinstance(state_str, list):
#                 if len(state_str) == 1:
#                     state_str = state_str[0]
...
#         introduction = f'You are an agent that can help me complete tasks on a smart phone. Given a task description, the descriptions for all elements on current UI screen, and the action histories you have completed, you predict the id of the UI element on this screen to perform the task, and the thought.'
#         # task_prompt = f"You are using {self.app} on a smartphone to {self.task}."
#         # state_prefix_prompt = 'the current GUI screen has the following elements:'
#         state_prefix_prompt = 'UI elements on the current screen: '
#         # history_prompt = f'You have completed the following steps, which should not be performed again: \n ' + ';\n '.join(action_history)
#         history_prompt = f'History:\n' + ';\n '.join(action_history)
#         # question = 'Which action should you choose next? Just return the element id and nothing else.\nIf no more action is needed, return -1.'
#         task_prompt = f'Task: {self.task}'
#         example = 'For example, you are Given: Task: create a new event type of deadlines.\n App: Calendar\n UI elements on the current screen: "<button id=0 checked=True>Regular event</button>\n <button id=1 checked=False>Add a new type</button>\n<button id=2 checked=False>Manage event types</button>\n<button id=3 class=\'ImageButton\'>go back</button>"\nYou should return: \n\{ID: 1. Thought: In order to add a new event type \'deadlines\', the answer should be to click \'Add a new type\' button to open the \'create new event\' interface and further define the event.\}\nHistory:\n- start the app Calendar;\n- <button class=\'Settings\' checked=False></button>.click();\n- <button checked=False>Manage event types</button>.click()'
#         if not is_text:
#             question = 'Return in the format of { ID: <element_id>. Thought: <the reason why you choose this element> }\nIf you think the task has been finished according to the action history, please return in the format of {-1, Thought: <the reason why you think the task has been finished>} '
#         else:
#             question = f'What text should you enter to the {view_text}? ' + 'Please answer strictly follow the format of: InputText: <text>' #'and thought in the format of: {Text: <input_text> Thought: <the reason why you choose to input this text>}.'
#         prompt = f'{introduction}\n{task_prompt}\n{state_prefix_prompt}\n{state_prompt}\n{history_prompt}\n{question}'
#         return prompt
...
#     def _get_action_from_views_actions(self, views, candidate_actions, state_strs, action_history):
#         '''
#         get action choice from LLM based on a list of views and corresponding actions
#         '''
#         views_with_id = []
#         for id in range(len(views)):
#             view_without_id = views[id]
#             view_without_id_list = view_without_id.split(' ', 1)
#             if len(view_without_id_list) < 2:
#                 view_without_id_list = view_without_id.split('>', 1)
#                 view_with_id = view_without_id_list[0] + f' id={len(views_with_id)}>' + view_without_id_list[1]
#             else:
#                 view_with_id = view_without_id_list[0] + f' id={len(views_with_id)} ' + view_without_id_list[1]
#             views_with_id.append(view_with_id)
#         state_prompt = '\n'.join(views_with_id)
#         prompt = self._make_prompt(state_prompt, action_history, is_text=False, state_str=state_strs)
        
#         print('********************************** prompt: **********************************')
#         print(prompt)
#         print('********************************** end of prompt **********************************')
#         response = input(f"UI element ID: ") if self.debug_mode else self._query_llm(prompt)
...
#         if '-1' in response:
#             self._save2yaml(file_name, state_prompt, -1, state_strs, inputs='null')
...
#         if isinstance(selected_action, SetTextEvent):
#             view_text = self._get_text_view_description(selected_action.view)
#             # question = f'What text should I enter to the {view_text}? Just return the text and nothing else.'
#             # prompt = f'{state_prompt}'
#             prompt = self._make_prompt(state_prompt, action_history, is_text=True, view_text=view_text, state_str=state_strs)
#             print(prompt)
#             response = input(f"input: ") if self.debug_mode else self._query_llm(prompt)
...
#         return selected_action, candidate_actions, selected_view_description
...
#             prompt = self._make_prompt(state_prompt, action_history, is_text=True, view_text=view_text, state_str=state_str)
#             # prompt = f'{introduction}\n{example}\n{task_prompt}\n{state_prefix_prompt}\n{state_prompt}\n\n{history_prompt}\n{question}'
#             print(prompt)
#             response = input(f"input into the element {view_text}: ") if self.debug_mode else self._query_llm(prompt)
