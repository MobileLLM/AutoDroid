...
class Memory(object):
    ...
    def __init__(self, app_name, app_output_path) -> None:
        ...
        if os.path.exists(utg_path): 
            ...
            self.raw_memory_graph = nx.DiGraph()
            for memory_graph in all_raw_memories:
                self.raw_memory_graph = nx.compose(self.raw_memory_graph, memory_graph)
    
    def get_first_state_str(self):
        '''
        get the first state_str of the utg (the first screen of the app)
        '''
        utg = os.path.join(self.app_output_path, 'utg.yaml')
        if not os.path.exists(utg):
            utg = os.path.join(self.app_output_path, 'utg1.yaml')
        ...
        # else:
        return data['records'][0]['new_state_str']
    
    def load_yaml_utg(self, update_state_strs, utg_path):
        '''
    ...
    def _get_all_utg_file_paths(self):
        all_files = []
        for root, dirs, files in os.walk(self.app_output_path):
            for file in files:
                if file[-5:] == '.yaml' and file[:3] == 'utg':
                    file_path = os.path.join(root, file)
                    all_files.append(file_path)
        return all_files
                

    def build_manual_memory_graph(self):
        '''
        ...
        for record_id in range(len(self.nodes_and_edges['records'])):
            ...
            if record_id < len(self.nodes_and_edges['records']) - 1:
                ...
                # if state_str == '557aeea88606609eec93a428b50eb5a9dd2eedef1a07e1fdde83fafc1625022c':
                #     print(state_str, self.nodes_and_edges['records'][record_id+1]['state_str'])
                raw_memory_graph.add_edge(state_str, self.nodes_and_edges['records'][record_id+1]['state_str'], event=events, label=str(events))
    ...
    def _find_item_target_screen(self, current_state_str, view_id):
        ...
        for successor_str in successors_of_current_view[current_state_str]:
    ...
    def _get_task_summary_by_gpt(self, state_str, state_items=None, target_item=None):
        ...
        if ui_element_num < 1:
            ...
            prefix_prompt = f'suppose you are using {self.app_name} app on a smartphone, given a GUI screen described in html, please summarize the function of this UI screen in phrases with verbs and nouns. Just return the function, do not tell me which button to click or which textbox to input, do not include \'go back to the front page\', or \'scroll up/down\' or any thing that you need to jump to another ui screen. Please ensure the summary is comprehensive, accurate, and concise'
            # request_prompt = f'\n List {ui_element_num - 1} things you can do by clicking or inputting text into the textbox. Just return the things you can do, do not tell me which button to click or which textbox to input, do not include \'go back to the front page\', or \'scroll up/down\' or any thing that you need to jump to another ui screen'
            ...
            prompt = prefix_prompt + example_request+example_summary+request_prompt+state_prompt+query_prompt
            print(prompt)
    ...
    def get_nl_desc(self, successor_depth=2):
        '''
        ...
        for state_id in tqdm(range(len(list(self.states.keys())))):
            ...
            # for node in state_successors_dict:
            #     print(node, state_successors_dict[node])
            # import pdb;pdb.set_trace()
            successor_new_items = {}
            ...
            for parent_node_str, successor_node_strs in state_successors_dict.items():
                ...
                if path_actions != []:  # the parent node is not the state_str itself 
                    ...
                    first_actions = path_actions[0]#.split(' or ')
                ...
                for successor_str in successor_node_strs:  # add all the successor's items into the elements' descriptions
                    ...
                    if path_actions == []:  # the parent node is the state_str itself 
